...
export function validateSiteEditTarget(
  site: SiteDocument,
  ...
  if (
    target.kind === "tool" &&
    !section.tools.some((tool) => tool.id === target.toolId)
  ) {
...
export function validatePlanAgainstTarget(
  site: SiteDocument,
  plan: PublicSitePlan,
  target: SiteEditTarget,
) {
  validateSiteEditTarget(site, target);
  if (digestValue(plan.target) !== digestValue(target))
...
export function validateBundleAgainstTarget(
  ...
  if (
    ...
    digestValue(reorderOperations[0]?.pageIds) !==
      digestValue(originalPageOrder)
  ) {
    throw new Error("delivery_scope_violation");
  }

  const meaningful = bundle.operations.filter(
    (operation) => operation.op !== "reorderPages",
  );
  ...
  if (sharedRegion) {
    ...
    if (
      meaningful.length !== 1 ||
      meaningful[0]?.op !== "replaceSharedRegion" ||
      meaningful[0].region !== region
    ) {
      throw new Error("delivery_scope_violation");
    }
    const value = meaningful[0].value;
    const other = region === "header" ? "footer" : "header";
    if (digestValue(value) === digestValue(originalSite.sharedShell[region]))
      throw new Error("delivery_scope_violation");
    if (
      bundle.operations.some(
        (operation) =>
          operation.op === "replaceSharedRegion" && operation.region === other,
      )
    ) {
      throw new Error("delivery_scope_violation");
    }
    if (target.kind !== "shared-region") {
      assertLocalPatchScope(
        originalSite,
        target,
        diffRegion(originalSite, region, value),
      );
    }
    return bundle;
  }
  ...
  if (
    meaningful.length !== 1 ||
    meaningful[0]?.op !== "updatePage" ||
    meaningful[0].pageId !== pageId
  ) {
    throw new Error("delivery_scope_violation");
  }
  if (
    meaningful[0].metadata &&
    Object.keys(meaningful[0].metadata).length > 0 &&
    target.kind !== "page"
  ) {
    throw new Error("delivery_scope_violation");
  }
...
function sectionOperationAllowed(
  operation: PagePatch[number],
  sectionId: string,
  toolIds: Set<string>,
) {
  if (operation.op === "updateSection")
    return operation.sectionId === sectionId;
  if (operation.op === "addTool")
    return operation.sectionId === sectionId && !toolIds.has(operation.tool.id);
  if (operation.op === "updateTool" || operation.op === "removeTool")
    return toolIds.has(operation.toolId);
  return false;
}

function toolOperationAllowed(
  operation: PagePatch[number],
  sectionId: string,
  toolId: string,
  toolIds: Set<string>,
  ...
  if (operation.op === "updateSection")
    return operation.sectionId === sectionId;
  if (operation.op === "updateTool")
    return operation.toolId === toolId && toolIds.has(operation.toolId);
...
function diffRegion(
  ...
  value: SiteDocument["sharedShell"]["header"],
): PagePatch {
