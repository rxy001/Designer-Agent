...
export const siteDeliveryPlanDraftSchema = z.object({
  ...
  navigation: z.object({ items: z.array(z.object({ label: z.string(), targetTaskKeyOrPageId: z.string() })) }),
...
export async function runSitePlanner(input: {
  ...
  const currentSite = {
    ...
    version: input.site.version,
    navigation: input.site.navigation,
    pages: input.site.pages.map((page) => ({
      id: page.id, title: page.title, route: page.route, version: page.body.version,
      sectionSummary: page.body.sections.map((section) => section.name),
    })),
    sharedShellSummary: {
      header: input.site.sharedShell.header.sections.map((section) => section.name),
      footer: input.site.sharedShell.footer.sections.map((section) => section.name),
    },
  };
  const response = await client.responses.parse({
    model: agentConfig.model.designerModel,
    store: agentConfig.model.storeResponses,
    instructions: "You are SitePlannerAgent. Plan only. For an existing target, provide pageId and set suggestedTitle/suggestedRoute to null. For a new target, set pageId to null and provide suggestedTitle/suggestedRoute. Reuse supplied page ids for existing targets, but never invent final ids for new pages. Never emit JSX, artifact paths, workspace paths, batches, or versions. Every schema field is required; use null for nullable title, route, target, or shared-copy values when no value is needed.",
